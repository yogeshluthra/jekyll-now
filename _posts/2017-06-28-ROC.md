...
(For purpose of illustrations, I will mainly assume that classifier learns a gaussian distribution for classes, although following analysis can be extended to any distribution. A $$ GaussianPDF(mean, std) = \frac{1}{\sqrt{2*\pi*std^2}} * e^{\frac{-1}{2} * (\frac{x-mean}{std})^2} $$)
...
![]({{site.url}}/images/2_gaussians.png)<!-- .element height="20%" width="20%" -->
